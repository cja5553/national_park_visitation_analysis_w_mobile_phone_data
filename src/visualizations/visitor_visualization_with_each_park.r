...
ggplot(model1, aes(x = date_range_start, y = raw_visitor_counts)) + ggtitle("Visitor Count to National Parks Jan 2018 - Apr 2021")+
  geom_line(aes(color = UNIT_NAME), 
            alpha = 0.5, position = position_dodge(0.8),size=1)+ylab("visitor count")+xlab("date")+theme(legend.position="bottom", legend.text=element_text(size=6), legend.title = element_blank(),axis.text.x = element_text(angle = 90),
